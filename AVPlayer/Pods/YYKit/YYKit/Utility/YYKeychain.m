...
static YYKeychainErrorCode YYKeychainErrorCodeFromOSStatus(OSStatus status)
{
    switch (status) {
        case errSecUnimplemented:
            return YYKeychainErrorUnimplemented;
        case errSecIO:
            return YYKeychainErrorIO;
        case errSecOpWr:
            return YYKeychainErrorOpWr;
        case errSecParam:
            return YYKeychainErrorParam;
        case errSecAllocate:
            return YYKeychainErrorAllocate;
        case errSecUserCanceled:
            return YYKeychainErrorUserCancelled;
        case errSecBadReq:
            return YYKeychainErrorBadReq;
        case errSecInternalComponent:
            return YYKeychainErrorInternalComponent;
        case errSecNotAvailable:
            return YYKeychainErrorNotAvailable;
        case errSecDuplicateItem:
            return YYKeychainErrorDuplicateItem;
        case errSecItemNotFound:
            return YYKeychainErrorItemNotFound;
        case errSecInteractionNotAllowed:
            return YYKeychainErrorInteractionNotAllowed;
        case errSecDecode:
            return YYKeychainErrorDecode;
        case errSecAuthFailed:
            return YYKeychainErrorAuthFailed;
        default:
            return 0;
    }
}
...
{
    if ([num isEqualToNumber:@NO])
        return YYKeychainQuerySynchronizationModeNo;
    if ([num isEqualToNumber:@YES])
        return YYKeychainQuerySynchronizationModeYes;
...
- (NSMutableDictionary *)queryDic
{
    ...
    if (self.account)
        dic[(__bridge id)kSecAttrAccount] = self.account;
    if (self.service)
        dic[(__bridge id)kSecAttrService] = self.service;
    ...
    if (![UIDevice currentDevice].isSimulator) {
        ...
        if (self.accessGroup)
            dic[(__bridge id)kSecAttrAccessGroup] = self.accessGroup;
    }
...
- (NSMutableDictionary *)dic
{
    ...
    if (self.service)
        dic[(__bridge id)kSecAttrService] = self.service;
    if (self.label)
        dic[(__bridge id)kSecAttrLabel] = self.label;
    ...
    if (self.accessible)
        dic[(__bridge id)kSecAttrAccessible] = YYKeychainAccessibleStr(self.accessible);
    if (self.passwordData)
        dic[(__bridge id)kSecValueData] = self.passwordData;
    if (self.type)
        dic[(__bridge id)kSecAttrType] = self.type;
    if (self.creater)
        dic[(__bridge id)kSecAttrCreator] = self.creater;
    if (self.comment)
        dic[(__bridge id)kSecAttrComment] = self.comment;
    if (self.descr)
        dic[(__bridge id)kSecAttrDescription] = self.descr;
                              ...
                              error:(NSError **)error
{
    if (!serviceName || !account) {
        if (error)
            *error = [YYKeychain errorWithCode:errSecParam];
        return nil;
    }
...
+ (BOOL)insertItem:(YYKeychainItem *)item error:(NSError **)error
{
    ...
    if (status != errSecSuccess) {
        if (error)
            *error = [YYKeychain errorWithCode:status];
        return NO;
    }
