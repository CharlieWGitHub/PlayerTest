...
{
    LOCK(NSDictionary *dic = [_dic entriesForKeys:keys])
    return dic;
}

- (NSString *)jsonStringEncoded
{
    LOCK(NSString *s = [_dic jsonStringEncoded])
    return s;
}

- (NSString *)jsonPrettyStringEncoded
{
    LOCK(NSString *s = [_dic jsonPrettyStringEncoded])
    return s;
}

- (id)popObjectForKey:(id)aKey
{
    LOCK(id o = [_dic popObjectForKey:aKey])
    return o;
}

- (NSDictionary *)popEntriesForKeys:(NSArray *)keys
{
    LOCK(NSDictionary *d = [_dic popEntriesForKeys:keys])
    return d;
}
