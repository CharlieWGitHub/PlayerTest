...
- (NSString *)plainTextForRange:(NSRange)range
{
    ...
    [self enumerateAttribute:YYTextBackedStringAttributeName
                     inRange:range
                     options:kNilOptions
...
- (BOOL)isSharedAttributesInAllRange
{
    ...
    [self enumerateAttributesInRange:self.rangeOfAll
    ...
    [self enumerateAttributesInRange:self.rangeOfAll
                          ...
                          usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
                            ...
                            if (attrs[(id)kCTForegroundColorAttributeName] && !attrs[NSForegroundColorAttributeName])
                                Fail;
                            if (attrs[(id)kCTStrokeColorAttributeName] && !attrs[NSStrokeColorAttributeName])
                                Fail;
                            ...
                            if (style && CFGetTypeID((__bridge CFTypeRef)(style)) == CTParagraphStyleGetTypeID())
                                Fail;
                          }];
    return result;
#undef Fail
