...
- (void)_updateSelectionView
{
    ...
    if (_markedTextRange) {
        ...
        if (rects)
            [allRects addObjectsFromArray:rects];
...
{
    if (!_state.trackingTouch || !_state.trackingGrabber)
        return;
...
- (void)_hideMagnifier
{
    ...
    if (_state.showingMagnifierCaret || _state.showingMagnifierRanged) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.15 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
          ...
          if (self)
              self->_state.ignoreTouchBegan = NO;
        });
    }
...
- (void)_scrollRangeToVisible:(YYTextRange *)range
{
            ...
            if (CGRectGetMinY(inter) > CGRectGetMinY(bounds)) { // keyboard below self.top
                ...
                if (originalContentInset.bottom < inter.size.height + extend) {
                    ...
                    [UIView animateWithDuration:0.25
                                          delay:0
                                        options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction | curve
                                     animations:^{
    ...
    if (!insetModified) {
        [UIView animateWithDuration:0.25
                              delay:0
                            options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationCurveEaseOut
                         animations:^{
...
- (void)_trackDidTickAutoScroll
{
    if (_autoScrollOffset != 0) {
        ...
        if (shouldScroll) {
            ...
            [UIView animateWithDuration:kAutoScrollMinimumDuration
                delay:0
...
- (YYTextRange *)_getClosestTokenRangeAtPoint:(CGPoint)point
{
    ...
    if (!touchRange)
        touchRange = [YYTextRange defaultRange];
...
{
    if (!_highlightable || !_innerLayout.containsHighlight)
        return nil;
    point = [self _convertPointToLayout:point];
...
{
    if (![self _isTextPositionValid:range.start])
        return NO;
    if (![self _isTextPositionValid:range.end])
        return NO;
    return YES;
}
...
- (YYTextPosition *)_correctedTextPosition:(YYTextPosition *)position
{
    if (!position)
        return nil;
    if ([self _isTextPositionValid:position])
        return position;
    if (position.offset < 0) {
...
- (YYTextRange *)_correctedTextRange:(YYTextRange *)range
{
    if (!range)
        return nil;
    if ([self _isTextRangeValid:range])
        return range;
...
- (void)_updateOuterProperties
{
    ...
    if (!style)
        style = _typingAttributesHolder.paragraphStyle;
    if (!style)
        style = [NSParagraphStyle defaultParagraphStyle];

    UIFont *font = _innerText.font;
    if (!font)
        font = _typingAttributesHolder.font;
    if (!font)
        font = [self _defaultFont];

    UIColor *color = _innerText.color;
    if (!color)
        color = _typingAttributesHolder.color;
    if (!color)
...
- (BOOL)_shouldDetectText
{
    ...
    if (!_highlightable)
        return NO;
    if (_linkTextAttributes.count == 0 && _highlightTextAttributes.count == 0)
        return NO;
    if (self.isFirstResponder || _containerView.isFirstResponder)
        return NO;
    return YES;
...
- (BOOL)_detectText:(NSMutableAttributedString *)text
{
    ...
    [_dataDetector enumerateMatchesInString:text.string
                                    options:kNilOptions
...
- (void)_saveToUndoStack
{
    ...
    if ([lastObject.text isEqualToAttributedString:self.attributedText])
        return;
...
- (BOOL)_canUndo
{
    ...
    if ([object.text isEqualToAttributedString:_innerText])
        return NO;
    return YES;
}
...
- (void)_showUndoRedoAlert NS_EXTENSION_UNAVAILABLE_IOS("")
{
    ...
    if (canUndo && canRedo) {
        if (kiOS8Later) {
            ...
            [alert addAction:[UIAlertAction actionWithTitle:strings[3]
                                                      style:UIAlertActionStyleDefault
                                                    handler:^(UIAlertAction *action) {
                                                      [_self _undo];
                                                      [_self _restoreFirstResponderAfterUndoAlert];
                                                    }]];
            [alert addAction:[UIAlertAction actionWithTitle:strings[2]
                                                      style:UIAlertActionStyleDefault
                                                    handler:^(UIAlertAction *action) {
                                                      [_self _redo];
                                                      [_self _restoreFirstResponderAfterUndoAlert];
                                                    }]];
            [alert addAction:[UIAlertAction actionWithTitle:strings[0]
...
#pragma clang diagnostic pop
        ...
        if (kiOS8Later) {
            ...
            [alert addAction:[UIAlertAction actionWithTitle:strings[1]
                                                      style:UIAlertActionStyleDefault
                                                    handler:^(UIAlertAction *action) {
                                                      [_self _redo];
                                                      [_self _restoreFirstResponderAfterUndoAlert];
...
{
    if (_linkTextAttributes == linkTextAttributes || [_linkTextAttributes isEqual:linkTextAttributes])
        return;
...
{
    if (_highlightTextAttributes == highlightTextAttributes || [_highlightTextAttributes isEqual:highlightTextAttributes])
        return;
...
{
    if (_exclusionPaths == exclusionPaths || [_exclusionPaths isEqual:exclusionPaths])
        return;
...
{
    if (_typingAttributes == typingAttributes || [_typingAttributes isEqual:typingAttributes])
        return;
...
- (void)_initTextView
{
    ...
    _linkTextAttributes = @{NSForegroundColorAttributeName : [self _defaultTintColor],
                            (id)
                            kCTForegroundColorAttributeName : (id)[self _defaultTintColor].CGColor};
...
{
    if (_highlightable == highlightable)
        return;
...
{
    if (_clearsOnInsertion == clearsOnInsertion)
        return;
...
- (void)setPlaceholderText:(NSString *)placeholderText
{
    if (_placeholderAttributedText.length > 0) {
        ...
        if (placeholderText.length > 0) {
            ...
            if (!_placeholderFont)
                _placeholderFont = _font;
            if (!_placeholderFont)
                _placeholderFont = [self _defaultFont];
...
- (BOOL)canBecomeFirstResponder
{
    ...
    if (!self.isEditable)
        return NO;
    if (_state.ignoreFirstResponder)
        return NO;
    if ([self.delegate respondsToSelector:@selector(textViewShouldBeginEditing:)]) {
        if (![self.delegate textViewShouldBeginEditing:self])
            return NO;
    }
    return YES;
}
...
- (void)paste:(id)sender
{
    ...
    if (!atr && _allowsPasteImage) {
        ...
        if (img && img.size.width > 1 && img.size.height > 1) {
            ...
            if (attrs)
                [attText addAttributes:attrs range:NSMakeRange(0, attText.length)];
            atr = attText;
        }
    }
...
- (void)_define:(id)sender
{
    ...
    [[self _getRootViewController] presentViewController:ref
                                                animated:YES
                                              completion:^{
                                              }];
}
...
- (void)setSelectedTextRange:(YYTextRange *)selectedTextRange
{
    ...
    if ([selectedTextRange isEqual:_selectedTextRange])
        return;
    [self _updateIfNeeded];
    [self _endTouchTracking];
    [self _hideMenu];
...
- (YYTextPosition *)positionWithinRange:(YYTextRange *)range atCharacterOffset:(NSInteger)offset
{
    ...
    if (offset == range.start.offset)
        ...
        return [YYTextPosition positionWithOffset:offset];
}
...
- (void)setFontIsBold_:(BOOL)fontBold
{
    ...
    if (!font)
        font = [self _defaultFont];
    if ([self fontIsBold_:font] == fontBold)
        return;
    if (fontBold) {
        font = [self boldFont_:font];
    }
