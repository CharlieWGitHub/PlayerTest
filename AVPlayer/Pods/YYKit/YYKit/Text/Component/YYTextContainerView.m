...
{
    if (_contentsFadeDuration == contentsFadeDuration)
        return;
    ...
    if (_attachmentChanged) {
        ...
        for (YYTextAttachment *a in _layout.attachments) {
            if ([a.content isKindOfClass:[UIView class]])
                [_attachmentViews addObject:a.content];
            if ([a.content isKindOfClass:[CALayer class]])
                [_attachmentLayers addObject:a.content];
        }
    }
}
