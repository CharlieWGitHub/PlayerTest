...
{
    if (!self._innerLayout.containsHighlight)
        return nil;
...
{
    if (_shadowColor == shadowColor || [_shadowColor isEqual:shadowColor])
        return;
...
{
    if (_textContainerPath == textContainerPath || [_textContainerPath isEqual:textContainerPath])
        return;
...
- (CGSize)intrinsicContentSize
{
    ...
    if (!_verticalForm) {
        ...
        if (containerSize.width == 0)
            containerSize.width = self.bounds.size.width;
    }
    else {
        containerSize.width = YYTextContainerMaxSize.width;
        containerSize.height = _preferredMaxLayoutWidth;
        if (containerSize.height == 0)
            containerSize.height = self.bounds.size.height;
    }
