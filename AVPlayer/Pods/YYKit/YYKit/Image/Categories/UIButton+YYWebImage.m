...
static inline NSArray *UIControlStateMulti(UIControlState state)
{
    ...
    if (state & UIControlStateHighlighted)
        [array addObject:@(UIControlStateHighlighted)];
    if (state & UIControlStateDisabled)
        [array addObject:@(UIControlStateDisabled)];
    if (state & UIControlStateSelected)
        [array addObject:@(UIControlStateSelected)];
    if ((state & 0xFF) == 0)
        [array addObject:@(UIControlStateNormal)];
    return array;
}
              ...
              completion:(YYWebImageCompletionBlock)completion
{
    ...
    dispatch_async_on_main_queue(^{
      ...
      if (imageFromMemory) {
          ...
          if (completion)
              completion(imageFromMemory, imageURL, YYWebImageFromMemoryCacheFast, YYWebImageStageFinished, nil);
          return;
      }
